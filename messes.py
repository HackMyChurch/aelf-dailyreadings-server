...
def postprocess_dedicace(version, mode, data):
    """
    The last sunday of October is the "dedicace des eglises". It only applies to
    a tiny subset of churches, but AELF made it the nominal case. This function
    prepends a nominal "romain" case.
    """
    if "DÉDICACE DES ÉGLISES" not in data["informations"].get(
        "jour_liturgique_nom", ""
    ):
        return

    # Hack: import here to workaround a circular dependency. Terrible design.
    from office_controller import get as do_get_office

    # Load the default variant
    office, _, _ = do_get_office(version, mode, "messes", data["date"], "romain")

    # Rename the original mass
    data["variants"][0]["name"] = "Dédicace des églises"

    # Prepend the generic version
    data["variants"] = office["variants"] + data["variants"]
...
def postprocess_links(version, mode, data):
    ...
    for variant_counter, variant in enumerate(variant_data):
        links += '<a href="%s#%s" class="variant-%s">%s</a>' % (
            base_link,
            variant["key"],
            variant_counter,
            variant["name"],
    ...
    for variant_counter, variant in enumerate(data["variants"]):
        text = links.replace(
            'class="variant-%s"' % variant_counter,
            'class="variant-%s active"' % variant_counter,
        )
        variant["lectures"].insert(
            0,
            [
                {
                    "title": "Messes",
...
def postprocess(version, mode, data):
    if (
        data["informations"].get("jour_liturgique_nom", "").lower().strip()
        == "samedi saint"
    ):
        return postprocess_holly_saturday(version, mode, data)

    postprocess_dedicace(version, mode, data)
